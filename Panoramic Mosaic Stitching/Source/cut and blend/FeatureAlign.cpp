              ...
              int nRANSAC, double RANSACthresh, CTransform3x3& M)
{
	...
	for(int count=0;count<nRANSAC;count++){
		matchId = rand()%(matches.size());
		idf1 = matches[matchId].id1;
		idf2 = matches[matchId].id2;
		temp[0][2] = f2[idf2-1].x - f1[idf1-1].x;
		temp[1][2] = f2[idf2-1].y - f1[idf1-1].y;

		int numOfIn = countInliers(f1,f2,matches,m,f,temp,RANSACthresh,inliersT);
                 ...
                 CTransform3x3 M, double RANSACthresh, vector<int> &inliers)
{
    ...
    for (unsigned int i=0; i<(int) matches.size(); i++) {
        ...
        //        These ids are 1-based indices into the feature arrays,
        //        so you access the appropriate features as f1[id1-1] and f2[id2-1].
		int translatedX = f1[matches[i].id1-1].x + M[0][2];
		int translatedY = f1[matches[i].id1-1].y + M[1][2];
		int disX = f2[matches[i].id2-1].x - translatedX;
		int disY = f2[matches[i].id2-1].y - translatedY;
		double error = sqrt(disX*disX+disY*disY);
