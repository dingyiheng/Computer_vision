 ...
 */
static void AccumulateBlend(CByteImage& img, CFloatImage& acc, CTransform3x3 M, double xBlend, float blendWidth)
{
    ...
    for (int y = bb_min_y; y <= bb_max_y; y++) {
        for (int x = bb_min_x; x < bb_max_x; x++) {
			...
			if(acc.Pixel(x, y, 3) >0 && acc.Pixel(x, y, 3)< 1){
				weight = (x_src - xBlend - blendWidth)*(1/blendWidth);
			}
			else if (x_src >= (img.Shape().width-blendWidth)){
				weight = weight = (img.Shape().width-x_src-1)*(1/blendWidth);
			}
			else if (acc.Pixel(x, y, 3) == 0){
...
CByteImage BlendImages(CImagePositionV& ipv, float blendWidth)
{
	...
	CTransform3x3 M_last;
	M_last.Translation(-min_x,-min_y);
    ...
    for (i = 0; i < n; i++) {
		...
		double xBlend = M_t.accEle(0,2) - M_last.accEle(0,2);
		M_last.setEle(0,2,M_t.accEle(0,2));

        CByteImage& img = ipv[i].img;

        // Perform the accumulation
		AccumulateBlend(img, accumulator, M_t ,xBlend, blendWidth);
	...
	//Deal with the CROP AND TRIM
	//if pano is false,not crop and not trim
	//if pano is true,crop and trim
	bool pano = false;
	int finalWidth = 0;
	int leftTrim = 0;
	if(pano){
		finalWidth = mShape.width - width;
		leftTrim = width/2;
	}
	else{
		finalWidth = mShape.width;
		leftTrim = 0;
	}

    // Allocate the final image shape
    //CShape cShape(mShape.width - width, height, nBands);
	CShape cShape(finalWidth, height, nBands);
	...
	//A[0][2] = width / 2;
	A[0][2] = leftTrim;
	A[1][0] = ( y_init-y_final )/cShape.width;
